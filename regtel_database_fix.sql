-- =====================================================
-- REGTEL DATABASE SCHEMA FIX
-- Fix for foreign key constraint error: jurisdiction_id (bigint) vs id (uuid)
-- =====================================================

-- First, create the jurisdictions table with correct BIGINT data type
-- This must be created BEFORE the regulatory_change_sources table
...
CREATE TABLE IF NOT EXISTS public.jurisdictions (
    ...
    compliance_complexity INTEGER DEFAULT 1, -- 1-10 scale
    api_endpoint TEXT,
    update_frequency INTEGER DEFAULT 24, -- hours
    is_active BOOLEAN DEFAULT true,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Create regulatory_frameworks table with proper foreign key
...
-- RegTel Platform - Complete Global Regulatory Database
-- Phase 1: All 58 Countries + Comprehensive Regulatory Framework
-- This creates the foundation for the world's most comprehensive regulatory intelligence platform

-- =====================================================
-- COMPLETE GLOBAL JURISDICTIONS (58 COUNTRIES)
-- =====================================================

-- Clear existing data and recreate with complete global coverage
TRUNCATE TABLE public.jurisdictions RESTART IDENTITY CASCADE;
...
-- =====================================================
-- REGULATORY CHANGE SOURCES TABLE (FIXED)
-- =====================================================

CREATE TABLE IF NOT EXISTS public.regulatory_change_sources (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    jurisdiction_id BIGINT REFERENCES public.jurisdictions(id),
    source_name VARCHAR(255) NOT NULL,
    source_url TEXT,
    source_type VARCHAR(100), -- website, rss, api, newsletter
    monitoring_frequency INTEGER DEFAULT 24, -- hours
    last_checked TIMESTAMPTZ,
    parsing_rules JSONB, -- Rules for extracting regulatory changes
    is_active BOOLEAN DEFAULT true,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Insert regulatory monitoring sources
...
-- Regulatory change monitoring indexes
...
-- Verify the fix worked by testing the foreign key constraint
SELECT 
    j.code,
    j.name,
    COUNT(rcs.id) as source_count
FROM public.jurisdictions j
LEFT JOIN public.regulatory_change_sources rcs ON j.id = rcs.jurisdiction_id
